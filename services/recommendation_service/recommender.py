...
def get_hybrid_recommendations(user, seed_track_id: str, num_recommendations: int = 10, context: str = "hybrid_general"):
    ...
    seen_track_ids = {seed_track_obj.id} # Don't recommend the seed track itself

    # Add content-based, ensuring no duplicates
    for track in content_based_recs:
        if track.id not in seen_track_ids:
            final_recommendations.append(track)
            seen_track_ids.add(track.id)

    # Add collaborative, ensuring no duplicates
    for track in collab_recs:
        if track.id not in seen_track_ids:
            final_recommendations.append(track)
            seen_track_ids.add(track.id)

    # If still not enough, could fall back to more general popular tracks or other strategies.
    # For now, just slice to the required number.
