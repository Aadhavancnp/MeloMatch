...
from .utils import convert_image_to_base64, get_tracks_by_same_artists, get_tracks_from_same_album # Added fallback functions
...
def track_detail(request, track_id):
    ...
    context = {
        'track': track,
        'recommendations': recommendations, # Main recommendations (already Track objects)
        'artists': artists,
        # Fallback logic starts here
        'fallback_recommendations': [], # Initialize
        'fallback_type': None,
    }

    if not recommendations: # If main recommendations are empty
        limit = 5 # Define a limit for fallback recommendations
        fallback_recommendations = []
        fallback_type = None

        # Try fetching by same artists
        fallback_artist_recs = get_tracks_by_same_artists(track.pk, limit=limit)
        if fallback_artist_recs:
            fallback_recommendations.extend(fallback_artist_recs)
            fallback_type = "artist"

        # If still not enough or none by artist, try by album
        if len(fallback_recommendations) < limit:
            needed = limit - len(fallback_recommendations)
            if needed > 0:
                fallback_album_recs = get_tracks_from_same_album(track.pk, limit=needed)
                if fallback_album_recs:
                    existing_ids = {fr.id for fr in fallback_recommendations}
                    for rec in fallback_album_recs:
                        if rec.id not in existing_ids:
                            fallback_recommendations.append(rec)
                            if not fallback_type: fallback_type = "album"
                            elif fallback_type == "artist": fallback_type = "artist_album" # Both yielded results
                    # Ensure we don't exceed limit, though individual calls already respect it.
                    # This is more for if you combined results from multiple fallbacks in a more complex way.
                    fallback_recommendations = fallback_recommendations[:limit]

        context['fallback_recommendations'] = fallback_recommendations
        context['fallback_type'] = fallback_type
