...
from services.spotify_service.client import (
    get_spotify_client, search_jiosaavn, get_track_details_jiosaavn,
    get_user_top_tracks, get_user_recently_played, create_playlist_spotify,
    search_tracks, get_or_create_playlist, get_playlist_tracks,
    extract_audio_features, download_preview, add_tracks_to_playlist_spotify,
    delete_playlist_spotify, remove_tracks_from_playlist_spotify,
    get_artist_details, get_artist_albums, get_artist_top_tracks
)
# get_recommendations was moved to recommendation_service.
...
def playlist_detail(request, playlist_id):
    # Assuming playlist_id is the DB primary key (e.g., Int or UUID)
    # If it can also be a Spotify ID, more complex lookup logic would be needed here.
    try:
        playlist = Playlist.objects.prefetch_related(
            'owner',
            'collaborators',
            'tracks__artists', # For track.artists_names
            'tracks__genres',  # For track.primary_genre
            # 'tracks__album' # If track.album is an FK to an Album model.
                            # Current Track model has album as CharField, so not needed.
        ).get(id=playlist_id)
    except Playlist.DoesNotExist:
        messages.error(request, "Playlist not found.")
        return redirect('dashboard') # Or a 404 page
    except ValueError: # If playlist_id is not a valid UUID/int
        messages.error(request, "Invalid playlist ID format.")
        return redirect('dashboard')


    # Permission check (using the method from the model)
    if not playlist.can_view(request.user):
        # If it's not public and user is not owner or collaborator
        messages.error(request, "You do not have permission to view this playlist.")
        # Consider redirecting to a generic 'access_denied' page or just dashboard
        return redirect('dashboard')

    can_edit_playlist = playlist.can_edit(request.user)
    collaborators_list = playlist.collaborators.all()

    # Spotify Syncing (Optional - can be kept if desired for owned playlists)
    # For collaborative playlists, direct Spotify sync might be complex (e.g., whose Spotify account to use?)
    # This part might need rethinking for purely collaborative, non-Spotify-linked playlists.
    # For now, let's assume if it has a spotify_id and an owner who is the current user, we can try to sync.
    if playlist.spotify_id and playlist.owner == request.user:
        try:
            sp = get_spotify_client(request)
            if sp: # Ensure client is available (user might not have linked Spotify)
                spotify_tracks_data = get_playlist_tracks(sp, playlist.spotify_id) # List of Track model instances

                # Efficiently update M2M if changed
                current_db_track_ids = set(playlist.tracks.values_list('id', flat=True))
                spotify_track_db_ids = {t.id for t in spotify_tracks_data if t is not None}

                if current_db_track_ids != spotify_track_db_ids:
                    playlist.tracks.set(spotify_tracks_data) # This replaces all tracks
                    # playlist.save() # .set() handles saving the M2M, no need for playlist.save() unless other fields change
                    logger.info(f"Synced tracks for Spotify playlist {playlist.name} (ID: {playlist.spotify_id}) owned by {request.user.username}")
                    # Re-fetch playlist to get fresh M2M tracks for the template if .set was called
                    playlist = Playlist.objects.prefetch_related(
                        'owner', 'collaborators', 'tracks__artists', 'tracks__genres'
                    ).get(id=playlist_id)

        except Exception as e:
            logger.error(f"Error syncing Spotify playlist {playlist.spotify_id} for user {request.user.username}: {e}")
            # Non-critical error, so we don't block page load.
            # messages.warning(request, "Could not fully sync with Spotify at the moment.")
    ...
    context = {
        ...
        'tracks': playlist.tracks.all() # Ensure tracks are passed, already prefetched.
    }
