...
class Playlist(models.Model):
    ...
    spotify_id = models.CharField(max_length=100, unique=True, db_index=True, null=True, blank=True) # Allowing system playlists without spotify ID for now
    description = models.TextField(null=True, blank=True)
    tracks = models.ManyToManyField(Track, related_name='member_of_playlists', blank=True) # Changed related_name, blank=True
    collaborators = models.ManyToManyField(CustomUser, related_name='collaborative_playlists', blank=True)
    image_url = models.URLField(null=True, blank=True)
    is_public = models.BooleanField(default=False) # For future use: public discoverable playlists
    ...
    # Add any methods related to collaboration, e.g., checking if a user is a collaborator or owner
