...
class PlaylistSettingsForm(forms.ModelForm):
    ...
    shared_with = forms.ModelMultipleChoiceField(
        queryset=User.objects.all(), # You might want to filter this further, e.g., exclude self, only friends/followers
    ...
    class Meta:
        model = Playlist
        fields = ['name', 'description', 'is_public', 'shared_with', 'image_url'] # Assuming image_url might be editable if it's not Spotify's
