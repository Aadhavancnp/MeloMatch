...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, 'Account created successfully. Welcome to MeloMatch!')
...
def user_login(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Clear any existing session data to prevent mixing user data
            request.session.flush()
...
def user_profile(request, username=None):
    if username:
        ...
        profile_user = get_object_or_404(
            User.objects.select_related('subscription__plan').prefetch_related(
                'following', 'followers'),
            username=username
        )
    else:
        ...
        profile_user = User.objects.select_related('subscription__plan').prefetch_related('following', 'followers').get(
            pk=current_user_pk)
    ...
    if request.method == 'POST' and is_own_profile:  # Profile update only for own profile
        ...
        profile_form = UserProfileForm(
            instance=profile_user) if is_own_profile else None
...
def settings(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Save theme preference to user's database
            theme = request.POST.get('theme')
            if theme in ['light', 'dark', 'system']:
                request.user.theme_preference = theme
                request.user.save()

            # Save language preference to session and cookie
            language = form.cleaned_data['language']
            translation.activate(language)
            request.session[translation.LANGUAGE_SESSION_KEY if hasattr(
                translation, 'LANGUAGE_SESSION_KEY') else '_language'] = language
        ...
        if not current_lang or current_lang == 'en-us':
            # Check session or cookie
            current_lang = request.session.get(
                '_language') or request.COOKIES.get('django_language', 'en')
...
def follow_user(request, username_to_follow):
    ...
    if request.user == user_to_follow:
        ...
        messages.success(
            request, f"You are now following {username_to_follow}.")
        UserActivity.objects.create(user=request.user, activity_type='follow',
                                    description=f"Started following {user_to_follow.username}")
        UserActivity.objects.create(user=user_to_follow, activity_type='new_follower',
                                    description=f"Is now followed by {request.user.username}")
...
def unfollow_user(request, username_to_unfollow):
    ...
    if request.user.following.filter(pk=user_to_unfollow.pk).exists():
        ...
        messages.success(
            request, f"You have unfollowed {username_to_unfollow}.")
        UserActivity.objects.create(user=request.user, activity_type='unfollow',
                                    description=f"Unfollowed {user_to_unfollow.username}")
    else:
        messages.warning(
            request, f"You are not following {username_to_unfollow}.")
