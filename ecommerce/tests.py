...
from music.models import Track, Genre  # Added Artist
...
class EcommerceTestCase(TestCase):
    ...
    def test_add_to_cart(self):
        ...
        response = self.client.post(
            reverse('add_to_cart', args=[self.track.pk]))
    ...
    def test_view_cart_with_items(self):
        ...
        CartItem.objects.create(cart=Cart.objects.create(
            user=self.user), track=self.track, quantity=2)

        response = self.client.get(reverse('view_cart'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, self.track.title)
        # Template uses "Order Summary" instead of "Cart Total"
    ...
    def test_remove_from_cart(self):
        ...
        cart_item = CartItem.objects.create(
            cart=cart, track=self.track, quantity=1)

        response = self.client.post(
            reverse('remove_from_cart', args=[cart_item.pk]))
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, reverse('view_cart'))
    ...
    def test_order_history_empty(self):
        ...
        self.assertEqual(response.status_code, 200)
        # Template uses "No orders yet" as heading
    ...
    def test_order_history_with_orders(self):
        ...
        order = Order.objects.create(
            user=self.user, total_price=self.track.price, status='completed')
        OrderItem.objects.create(
            order=order, track=self.track, price=self.track.price, quantity=1)

        response = self.client.get(reverse('order_history'))
        self.assertEqual(response.status_code, 200)
        # Template shows "#1" in table cell for order ID
    ...
    def test_order_detail(self):
        ...
        self.assertContains(response, f"${order_item.price:.2f}")
    ...
    def test_order_detail_unauthorized(self):
        ...
        order = Order.objects.create(
            user=other_user, total_price=10.00, status='completed')
