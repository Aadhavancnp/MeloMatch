...
def create_subscription_checkout_session(request, plan_id):
    ...
    if not plan.stripe_price_id:
        ...
        return JsonResponse({'error': 'Configuration error: Subscription plan is not linked to a Stripe Price ID.'},
                            status=500)
    ...
    try:
        ...
        return JsonResponse({'error': 'Could not initiate subscription. Please try again later or contact support.'},
                            status=500)
